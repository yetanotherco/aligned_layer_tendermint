...
resource "hcloud_server" "alignedlayer-genesis-runner" {
  ...
  user_data = <<EOF
    ...
    runcmd:
      ...
      - printf "${var.password}\n" | alignedlayerd genesis gentx victor ${var.genesis_staking_amount}${var.staking_token} --account-number 0 --sequence 0 --chain-id ${var.chain_id} --gas 1000000 --gas-prices 0.1${var.staking_token}

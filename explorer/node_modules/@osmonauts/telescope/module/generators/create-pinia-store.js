...
export const plugin = (builder, bundler) => {
  ...
  if (queryContexts.length > 0) {
    ...
    queryContexts.forEach(c => {
      ...
      if (!enabled) return;
      const includePinia = c.proto.pluginValue('pinia.enabled') && isRefIncluded(c.ref, c.proto.pluginValue('pinia.include'));
      ...
      const prog = [].concat([...imports, ...piniaImport]).concat(ctx.body).concat(ast);

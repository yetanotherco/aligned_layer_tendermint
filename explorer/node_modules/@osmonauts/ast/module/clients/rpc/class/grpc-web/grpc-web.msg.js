...
const rpcClassMethod = (context, name, msg, svc, packageImport) => {
  ...
  let methodArgs = identifier('request', t.tsTypeAnnotation(t.tsTypeReference(t.identifier(partialName), t.tsTypeParameterInstantiation([t.tsTypeReference(t.identifier(requestType))]))));
  const metadataArgs = metadata;
  const fieldNames = Object.keys(svc.fields ?? {});
  const hasParams = fieldNames.length > 0;

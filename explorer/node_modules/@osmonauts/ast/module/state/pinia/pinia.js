...
function buildState(methods) {
  const fields = methods.map(x => t.objectProperty(t.identifier(x.name), t.tsAsExpression(t.identifier("{}"), t.tsUnionType([t.tsTypeReference(t.identifier(`${x.method.responseType}SDKType`))]))));
...
export const createPiniaStore = (context, service) => {
  ...
  return t.exportNamedDeclaration(t.variableDeclaration('const', [t.variableDeclarator(t.identifier(storeName), t.callExpression(t.identifier('defineStore'), [t.identifier(`'${key}'`), t.objectExpression([t.objectProperty(t.identifier('state'), buildState(methods)), t.objectProperty(t.identifier('getters'), buildGetter(methods)), t.objectProperty(t.identifier('actions'), buildAction(methods))])]))]));
};
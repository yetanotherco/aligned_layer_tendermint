...
const makeFunctionWrapper = (functionName, stmt) => {
  return t.exportNamedDeclaration(t.variableDeclaration('const', [t.variableDeclarator(t.identifier(functionName), t.arrowFunctionExpression([identifier('content', t.tsTypeAnnotation(t.tsTypeReference(t.identifier('AnyAmino'))))], t.blockStatement([stmt])))]));
};
...
export const createInterfaceFromAminoHelper = (context, functionName, typeRefs) => {
  ...
  if (!switchCases.length) {
    ...
    ast = t.switchStatement(t.memberExpression(t.identifier('content'), t.identifier('type')), [...switchCases,
    /// default

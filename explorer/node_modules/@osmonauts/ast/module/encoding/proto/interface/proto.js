...
export const createProtoType = (context, name, proto, type = 'Msg') => {
  ...
  const fields = [];
  if (context.pluginValue('prototypes.addTypeUrlToDecoders') && (context.pluginValue('interfaces.enabled') && proto.options?.['(cosmos_proto.implements_interface)'] || context.ref.proto.package === 'google.protobuf' && name === 'Any')) {
    fields.push(tsPropertySignature(t.identifier('$typeUrl'), t.tsTypeAnnotation(t.tsStringKeyword()), true));
  }
  ...
  [].push.apply(fields, Object.keys(proto.fields).reduce((m, fieldName) => {
    ...
    const propSig = tsPropertySignature(t.identifier(fieldNameWithCase), t.tsTypeAnnotation(protoField), optional || getFieldOptionality(context, field, isOneOf));
    const comments = [];
    if (field.comment &&
...
export const createProtoTypeType = (context, name, proto) => {
  ...
  return t.exportNamedDeclaration(t.tsInterfaceDeclaration(t.identifier(ProtoMsgName), null, [], t.tsInterfaceBody([tsPropertySignature(t.identifier(context.options.prototypes.parser.keepCase ? 'type_url' : 'typeUrl'), t.tsTypeAnnotation(typ), false), tsPropertySignature(t.identifier('value'), t.tsTypeAnnotation(t.tsTypeReference(t.identifier('Uint8Array'))), false)])));
};
export const createProtoInterfaceEncodedType = (context, name, proto) => {
  ...
  const fieldsWithInfo = Object.keys(proto.fields).reduce((m, fieldName) => {
    const isOneOf = oneOfs.includes(fieldName);
    const field = proto.fields[fieldName];

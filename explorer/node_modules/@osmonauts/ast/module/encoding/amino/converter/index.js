...
export const createAminoConverterItem = ({
  ...
  const toAmino = context.options.aminoEncoding.useRecursiveV2encoding ? t.memberExpression(t.identifier(proto.name), t.identifier('toAmino')) : toAminoJsonMethod({
    context,
    proto
  });
  const fromAmino = context.options.aminoEncoding.useRecursiveV2encoding ? t.memberExpression(t.identifier(proto.name), t.identifier('fromAmino')) : fromAminoJsonMethod({
    context,
    proto
  });
